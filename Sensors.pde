class SensorSystem {
  ...
  int     laps                  = 0;
  boolean hasHitCheckpoint      = false;
  ...
  void updateSensorsignals(PVector pos, PVector vel) {
    ...
    if (lastGreenDetection && !currentGreenDetection && hasHitCheckpoint) {  //sidst grønt - nu ikke -vi har passeret målstregen 
    ...
    if (!hasHitCheckpoint && pos.x>width/2) {
